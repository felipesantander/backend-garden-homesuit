...
from .register import ConfigurationItemSerializer
...
class MachineUpdateSerializer(serializers.Serializer):
    ...
    def validate_serial(self, value):
        # Allow same serial if updating same machine
        # The view should pass the machine instance through 'self.instance'
        instance = self.instance
        queryset = Machine.objects.filter(serial=value)
        if instance:
            queryset = queryset.exclude(machineId=instance.machineId)
